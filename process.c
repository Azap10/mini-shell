...
int pipeCase(const CMD *cmdList, bool BG) {
    ...
    if (fork_status_left == 0) {
        ...
        close(fd[0]);
        close(fd[1]);
        process_status_left = process(cmdList->left);
        exit(process_status_left);
    }
        ...
        if (fork_status_right == 0) {
            ...
            close(fd[0]);
            close(fd[1]);
            process_status_right = process(cmdList->right);
            exit(process_status_right);
        }
            ...
            if (STATUS(process_status_right) != 0) {
                ...
                // setenv("?", exit_status, 1);
                return STATUS(process_status_right);
            }
            // sprintf(exit_status, "%d", STATUS(process_status_left));
            // setenv("?", exit_status, 1);
            return STATUS(process_status_left);
        }
    }
}
...
int background_process(const CMD *cmdList) {
    int fork_status = fork();
    if (fork_status == 0) {
        int exit_status = process(cmdList);
